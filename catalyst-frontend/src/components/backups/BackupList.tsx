    ...
    <div className="space-y-3">
      {sorted.map((backup) => {
        ...
        return (
          <div
            key={backup.id}
            className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/60 px-4 py-4"
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{backup.name}</div>
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                {backup.download ? (
                  <button
                    className="rounded-md border border-slate-200 dark:border-slate-800 px-3 py-1 text-xs font-semibold text-slate-600 dark:text-slate-200 hover:border-slate-200 dark:border-slate-700 disabled:opacity-60"
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-300 sm:grid-cols-5">
              ...
              <div className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2">
                <div className="text-slate-500 dark:text-slate-400">Checksum</div>
                <div className="text-[11px] text-slate-600 dark:text-slate-200">
                  {backup.checksum ? `${backup.checksum.slice(0, 12)}...` : 'n/a'}
                </div>
              </div>
              <div className="rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-2">
                <div className="text-slate-500 dark:text-slate-400">Path</div>
                <div className="text-[11px] text-slate-600 dark:text-slate-200 truncate">{backup.path}</div>
              </div>
            </div>
          </div>
        );
